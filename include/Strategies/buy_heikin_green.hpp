...
private:
    ...
    void registerFilters() {
        ...
        if (config.use_previous_ha_candle_red_filter)
            active_filters.push_back([this]() {
                return Filters::previousHACandlesRed(*candle_manager.get(), config.previous_ha_candle_red_filter_n, 1, "Bougie HA précédente", logger.get());
            });
...
public:
    BuyHeikinGreen(const StrategyBaseConfig& base_cfg, const BuyHeikinGreenConfig& bhg_cfg) 
        ...
        atrlog_calculator = std::make_shared<ATR>(ATRParams(base_cfg.atr_period, true));
        atrlog_filter_calculator = std::make_shared<ATR>(ATRParams(config.atr_filter_period, true));
